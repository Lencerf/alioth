...
impl KvmIrqFd {
    ...
    fn update_routing_table(&self, table: &HashMap<u32, KvmMsiEntryData>) -> Result<()> {
        ...
        for index in 0..24 {
            entries[index].gsi = index as u32;
        ...
        log::trace!("gsi routing updated to {irq_routing:#x?}");
        Ok(())
    }
}
...
impl IrqFd for KvmIrqFd {
    impl_irqfd_method!(get_addr_lo, set_addr_lo, addr_lo);
    impl_irqfd_method!(get_addr_hi, set_addr_hi, addr_hi);
    impl_irqfd_method!(get_data, set_data, data);
