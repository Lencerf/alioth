...
fn search_pvh_note<F: Read + Seek>(
    ...
    while pos < size {
        ...
        file.seek(SeekFrom::Current(align_up!(header.name_sz as u64, align_bits) as i64))?;
...
pub fn load<P: AsRef<Path>>(
    ...
    kernel.read_exact(elf_header.as_mut_bytes()).context(access_kernel)?;
    ...
    kernel.seek(SeekFrom::Start(elf_header.ph_off)).context(access_kernel)?;
    let mut program_header =
        Elf64ProgramHeader::new_vec_zeroed(elf_header.ph_num as usize).unwrap();
    kernel.read_exact(program_header.as_mut_bytes()).context(access_kernel)?;
    for program_header in program_header.iter() {
        ...
        if program_header.file_sz > 0 {
            ...
            kernel.seek(SeekFrom::Start(program_header.offset)).context(access_kernel)?;
    ...
    if pvh_entry.is_none() && elf_header.sh_num > 0 {
        kernel.seek(SeekFrom::Start(elf_header.sh_off)).context(access_kernel)?;
        let mut sections = Elf64SectionHeader::new_vec_zeroed(elf_header.sh_num as usize).unwrap();
        kernel.read_exact(sections.as_mut_bytes()).context(access_kernel)?;
        for section in sections.iter() {
            if section.type_ != SHT_NOTE {
                continue;
            }
            pvh_entry =
                search_pvh_note(&mut kernel, section.offset, section.size, section.addr_align)
                    .context(access_kernel)?;
    ...
    if let Some(cmdline) = cmdline {
        ...
        if cmdline.len() as u64 > KERNEL_CMDLINE_LIMIT {
            return error::CmdLineTooLong { len: cmdline.len(), limit: KERNEL_CMDLINE_LIMIT }
                .fail();
        }
    ...
    for (addr, region) in mem_regions.iter() {
        ...
        start_info_page.memory_map[index] =
            HvmMemmapTableEntry { addr: *addr, size: region.size, type_, reserved: 0 };
        index += 1;
    }
    ...
    // set up gdt
    let boot_cs =
        SegRegVal { selector: 0x10, base: 0, limit: 0xfff_ffff, access: SegAccess(0xc09b) };
    let boot_ds =
        SegRegVal { selector: 0x18, base: 0, limit: 0xfff_ffff, access: SegAccess(0xc093) };
    let boot_tr = SegRegVal { selector: 0x20, base: 0, limit: 0x67, access: SegAccess(0x8b) };
    ...
    Ok(InitState {
        ...
        sregs: vec![(SReg::Cr0, Cr0::PE.bits() as u64), (SReg::Cr4, 0), (SReg::Efer, 0)],
        seg_regs: vec![
            (SegReg::Cs, boot_cs),
            (SegReg::Ds, boot_ds),
            (SegReg::Es, boot_ds),
