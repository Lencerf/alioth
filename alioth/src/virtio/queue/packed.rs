...
impl<'q, 'm> PackedQueue<'q, 'm> {
    pub fn new(
        ...
        unsafe {
            (&mut *notification).flag = EventFlag::ENABLE;
        }
        Ok(Some(PackedQueue {
            reg,
            ram,
    ...
    fn set_flag_used(&self, flag: &mut DescFlag) {
        if self.used_index.wrap_counter() {
...
impl<'m> VirtQueuePrivate<'m> for PackedQueue<'_, 'm> {
    ...
    fn wrapping_add(&self, index: WrappedIndex, count: u16) -> WrappedIndex {
        index.wrapping_add(count, self.size)
    }
}
