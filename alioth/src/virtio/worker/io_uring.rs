    ...
    E: IoeventFd,
{
        ...
        D: VirtioIoUring,
    {
        ...
        let queue_submit = self.queue_submits.get_mut(q_index as usize).unwrap();

        q.handle_desc(q_index, self.irq_sender, |index, desc| {
            ...
            match dev.handle_desc(q_index, desc)? {
                BufferAction::Sqe(sqe) => {
                    let buffer_key = ((index as u32) << 16) | q_index as u32;
    ...
    E: IoeventFd,
{
    ...
    fn handle_event(&mut self, dev: &mut D, event: &Self::Event) -> Result<()> {
        ...
        if token & TOKEN_DESCRIPTOR == TOKEN_DESCRIPTOR {
            ...
            let desc_index = (buffer_key >> 16) as u16;
            ...
            queue_submit.count -= 1;
            queue.handle_pending(desc_index, q_index, self.irq_sender, |desc| {
                dev.complete_desc(q_index, desc, event)
            })?;
