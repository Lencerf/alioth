...
impl Drop for Net {
    fn drop(&mut self) {
        println!(
            ...
            self.usefull_tx,
            self.useless_tx,
            (self.usefull_tx as f64) / (self.useless_tx as f64)
        );
        println!(
            "rx: {}/{} = {}",
            self.usefull_rx,
            self.useless_rx,
            (self.usefull_rx as f64) / (self.useless_rx as f64)
        );
        println!(
            "read: {}/{} = {}",
            self.usefull_read,
            self.useless_read,
            (self.usefull_read as f64) / (self.useless_read as f64)
        );
    }
}
    ...
    pub fn new(param: NetParam, name: Arc<String>) -> Result<Self> {
        ...
        let net = Net {
            ...
            config: Arc::new(NetConfig {
                mac: param.mac,
                max_virtqueue_pairs: param.queue_pairs,
                mtu: param.mtu,
                ..Default::default()
            }),
            useless_rx: 0,
...
impl Virtio for Net {
    ...
    fn num_queues(&self) -> u16 {
        let data_queues = self.config.max_virtqueue_pairs << 1;
    ...
    fn handle_queue(
        ...
        if index == self.config.max_virtqueue_pairs * 2 {
            unimplemented!()
        } else if index & 1 == 0 {
