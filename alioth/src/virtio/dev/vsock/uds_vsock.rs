...
impl UdsVsock {
    ...
    fn handle_conn_request<'a, 'm, Q, S>(
        &mut self,
        token: Token,
        socket: UnixStream,
        rx_q: &mut Q,
    ...
    fn respond_rst<'m, Q, S>(&self, hdr: &VsockHeader, irq_sender: &S, rx_q: &mut Q) -> Result<()>
    where
        Q: VirtQueue<'m>,
        S: IrqSender,
    {
    ...
    fn respond<'m, Q, S>(&self, hdr: &VsockHeader, irq_sender: &S, rx_q: &mut Q) -> Result<()>
    where
        Q: VirtQueue<'m>,
        S: IrqSender,
    {
        let mut hdr_buf = hdr.as_bytes();
        handle_desc(&self.name, VsockVirtq::RX.raw(), rx_q, irq_sender, |desc| {
            if hdr_buf.is_empty() {
                return Ok(None);
            }
            let c = hdr_buf.read_vectored(&mut desc.writable)?;
            Ok(Some(c))
        })?;
        if !hdr_buf.is_empty() {
            log::error!(
        ...
        S: IrqSender,
    {
        ...
        match hdr.op {
            ...
            _ => {
                log::info!("{hdr:02x?}, {body:02x?}");
                let r_sizes: Vec<_> = desc.readable.iter().map(|s| s.len()).collect();
                let w_sizes: Vec<_> = desc.writable.iter().map(|s| s.len()).collect();
                log::info!("readable sizes: {:?}", r_sizes);
                log::info!("writable sizes: {:?}", w_sizes);
                log::info!("readable: {:02x?}", desc.readable);
                log::info!("writable: {:02x?}", desc.writable);
                unimplemented!("Unsupported operation: {:?}", hdr.op);
            }
        }
    }

    fn handle_tx<'a, 'm, Q, S, E>(
        &mut self,
        ...
        E: IoeventFd,
    {
        ...
        handle_desc(&name, VsockVirtq::TX.raw(), tx_q, irq_sender, |desc| {
        ...
        S: IrqSender,
    {
        ...
        handle_desc(&self.name, rx_idx, rx_q, irq_sender, |desc| {
            let nread = copy_to_rx(&mut hdr, &mut conn.reader, &mut desc.writable)?;
            if nread == 0 {
                return Ok(None);
            }
            log::trace!(
...
impl Virtio for UdsVsock {
    ...
    fn ioeventfd_offloaded(&self, _: u16) -> Result<bool> {
        Ok(false)
    }
