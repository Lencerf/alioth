...
use crate::vm::{Context, Result, error};
    ...
    V: Vm,
{
    pub(crate) fn create_hob(&self, dst: &mut [u8], mut accepted: Vec<(u64, u64)>) -> Result<u64> {
        let hob_phys = self.board.arch.tdx_hob.load(Ordering::Relaxed);
        let mut entries = self.board.memory.mem_region_entries();
        ...
        for entry in entries {
            match entry.r#type {
                TdvfSectionType::TD_HOB => {
                    ...
                    hob_ram = Some(p);
                    let tdx_hob = &self.board.arch.tdx_hob;
    ...
    pub(crate) fn tdx_init_ap(&self) -> Result<()> {
        let hob = self.board.arch.tdx_hob.load(Ordering::Relaxed);
        self.vcpu.tdx_init_vcpu(hob)?;
        Ok(())
    }

    pub(crate) fn tdx_finalize(&self) -> Result<()> {
        self.board.vm.tdx_finalize_vm()?;
        Ok(())
    }
}
