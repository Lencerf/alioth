...
use crate::vm::{Context, Result, VcpuHandle};

impl<V: Vm> Context<V> {
    pub(crate) fn init_vcpu(&mut self) -> Result<()> {
        let apic_id = self.board.encode_cpu_identity(self.index) as u32;
        let mut cpuids = self.board.arch.cpuids.clone();
        ...
        let msrs = [(IA32_MISC_ENABLE, MiscEnable::FAST_STRINGS.bits())];
        self.vcpu.set_msrs(&msrs)?;
        Ok(())
    }

    pub(crate) fn init_boot_vcpu(&mut self, init: &InitState) -> Result<()> {
        if matches!(self.board.config.coco, Some(Coco::IntelTdx { .. })) {
            return Ok(());
        }
        self.vcpu
    ...
    pub(crate) fn init_ap(&mut self, vcpus: &[VcpuHandle]) -> Result<()> {
        let Some(coco) = &self.board.config.coco else {
            return Ok(());
        };
    ...
    pub(crate) fn setup_firmware(&self, fw: &Path, payload: &Payload) -> Result<InitState> {
        let (init_state, mut rom) = firmware::load(&self.board.memory, fw)?;
        self.setup_coco(&mut rom)?;
        self.board.setup_fw_cfg(payload)?;
        Ok(init_state)
    }
