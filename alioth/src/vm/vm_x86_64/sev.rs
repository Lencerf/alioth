    ...
    V: Vm,
{
    fn fill_snp_cpuid(&self, entries: &mut [SnpCpuidFunc]) {
        for ((in_, out), dst) in zip(self.board.arch.cpuids.iter(), entries.iter_mut()) {
            dst.eax_in = in_.func;
    ...
    fn update_snp_desc(&self, desc: &SevMetadataDesc) -> Result<()> {
        ...
        let page_type = match desc.type_ {
            ...
            SevDescType::CPUID => {
                ...
                assert!(cpuid_table.entries.len() >= self.board.arch.cpuids.len());
                cpuid_table.count = self.board.arch.cpuids.len() as u32;
        ...
        let ret = self
            .board
            .vm
            .snp_launch_update(bytes, desc.base as _, page_type);
    ...
    pub(crate) fn setup_sev(&self, fw: &mut ArcMemPages, policy: SevPolicy) -> Result<()> {
        self.board.memory.register_encrypted_pages(fw)?;

        let data = fw.as_slice_mut();
        if policy.es() {
            self.parse_sev_api_eip(data)?;
        }
        self.board.vm.sev_launch_update_data(data)?;
        Ok(())
    }
    ...
    pub(crate) fn sev_finalize(&self, policy: SevPolicy) -> Result<()> {
        if policy.es() {
            self.board.vm.sev_launch_update_vmsa()?;
        }
        self.board.vm.sev_launch_measure()?;
        self.board.vm.sev_launch_finish()?;
        Ok(())
    }

    pub(crate) fn snp_finalize(&self) -> Result<()> {
        self.board.vm.snp_launch_finish()?;
        Ok(())
    }

    pub(crate) fn sev_init_ap(&mut self) -> Result<()> {
        let eip = self.board.arch.sev_ap_eip.load(Ordering::Acquire);
