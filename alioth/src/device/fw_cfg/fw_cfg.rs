...
impl FwCfg {
    ...
    pub fn add_board_info(&mut self, config: &BoardConfig) {
        self.known_items[FW_CFG_NB_CPUS as usize] = FwCfgContent::Lu16(config.cpu.count.into());
        self.known_items[FW_CFG_RAM_SIZE as usize] = FwCfgContent::Lu64(config.mem.size.into());
        log::info!("known items: {:x?}", self.known_items);
    }
