...
use crate::loader::{firmware, Executable, InitState, Payload};
        ...
        H: Hypervisor<Vm = V>,
    {
        ...
        for (in_, out) in &mut cpuids {
            if in_.func == 0x1 {
                ...
                if matches!(&config.coco, Some(Coco::AmdSev { .. } | Coco::AmdSnp { .. })) {
        ...
        Ok(Self { cpuids, sev_ap_eip: AtomicU32::new(0), _phantom: PhantomData })
    }
}
    ...
    V: Vm,
{
    ...
    fn update_snp_desc(&self, offset: usize, fw_range: &mut [u8]) -> Result<()> {
        ...
        self.memory.mark_private_memory(desc.base as _, desc.len as _, true)?;
        let mut ret = self.vm.snp_launch_update(range_bytes, desc.base as _, snp_page_type);
        if ret.is_err() && desc.type_ == SEV_DESC_TYPE_CPUID {
            ...
            ret = self.vm.snp_launch_update(range_bytes, desc.base as _, snp_page_type);
        }
        ret?;
        Ok(())
    }
    ...
    fn setup_coco(&self, fw: &mut ArcMemPages) -> Result<()> {
        ...
        match coco {
            ...
            Coco::AmdSnp { .. } => {
                ...
                self.memory.mark_private_memory(fw_gpa, fw_range.len() as _, true)?;
                self.vm.snp_launch_update(fw_range, fw_gpa, SnpPageType::Normal).unwrap();
            }
        }
        Ok(())
    }
    ...
    pub fn create_ram(&self) -> Result<()> {
        ...
        let region_low = MemRegion {
            ...
            entries: vec![
                MemRegionEntry { size: BIOS_DATA_END, type_: MemRegionType::Reserved },
                MemRegionEntry { size: EBDA_START - BIOS_DATA_END, type_: MemRegionType::Ram },
                MemRegionEntry { size: EBDA_END - EBDA_START, type_: MemRegionType::Acpi },
                MemRegionEntry { size: low_mem_size - EBDA_END, type_: MemRegionType::Ram },
            ],
    ...
    T: FromBytes,
{
    ...
    let offset_table_end = offset_table_len
        .checked_sub((table_len as usize).checked_sub(size_of::<u16>())?.checked_sub(GUID_SIZE)?)?;
