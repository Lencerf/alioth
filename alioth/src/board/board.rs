    ...
    V: Vm,
{
    ...
    fn vcpu_loop(&self, vcpu: &mut <V as Vm>::Vcpu, index: u32) -> Result<bool, Error> {
    ...
    fn create_vcpu(&self, index: u32, event_tx: &Sender<u32>) -> Result<V::Vcpu> {
        let identity = self.encode_cpu_identity(index as u16);
        let vcpu = self
            .vm
            .create_vcpu(index as u16, identity)
