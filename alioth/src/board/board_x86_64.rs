    ...
    V: Vm,
{
    ...
    pub fn init_ap(&self, index: u32, vcpu: &mut V::Vcpu, vcpus: &VcpuGuard) -> Result<()> {
    ...
    pub fn init_vcpu(&self, index: u32, vcpu: &mut V::Vcpu) -> Result<()> {
    ...
    pub fn reset_vcpu(&self, _index: u32, _vcpu: &mut V::Vcpu) -> Result<()> {
        Ok(())
    }
    ...
    pub fn coco_init(&self, index: u32) -> Result<()> {
        if index != 0 {
            return Ok(());
        }
        if let Some(coco) = &self.config.coco {
    ...
    pub fn coco_finalize(&self, index: u32, vcpus: &VcpuGuard) -> Result<()> {
        if let Some(coco) = &self.config.coco {
            self.sync_vcpus(vcpus)?;
            if index == 0 {
                match coco {
