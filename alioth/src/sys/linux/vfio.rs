...
ioctl_writeread!(vfio_device_get_info, ioctl_io(VFIO_TYPE, 107), VfioDeviceInfo);
ioctl_writeread!(vfio_device_get_region_info, ioctl_io(VFIO_TYPE, 108), VfioRegionInfo);
ioctl_writeread!(vfio_device_get_irq_info, ioctl_io(VFIO_TYPE, 109), VfioIrqInfo);

ioctl_write_buf!(vfio_device_set_irqs, ioctl_io(VFIO_TYPE, 110), VfioIrqSet);

ioctl_none!(vfio_device_reset, VFIO_TYPE, 111, 0);

ioctl_write_ptr!(vfio_device_bind_iommufd, ioctl_io(VFIO_TYPE, 118), VfioDeviceBindIommufd);
...
ioctl_writeread!(iommu_ioas_alloc, ioctl_io(IOMMUFD_TYPE, 0x81), IommuIoasAlloc);

ioctl_write_ptr!(iommu_ioas_map, ioctl_io(IOMMUFD_TYPE, 0x85), IommuIoasMap);

ioctl_write_ptr!(iommu_ioas_unmap, ioctl_io(IOMMUFD_TYPE, 0x86), IommuIoasUnmap);

ioctl_write_ptr!(vfio_iommu_map_dma, ioctl_io(VFIO_TYPE, 113), VfioIommuType1DmaMap);

ioctl_writeread!(vfio_iommu_unmap_dma, ioctl_io(VFIO_TYPE, 114), VfioIommuType1DmaUnmap);

ioctl_write_val!(vfio_group_get_device_fd, ioctl_io(VFIO_TYPE, 106), *const libc::c_char);
